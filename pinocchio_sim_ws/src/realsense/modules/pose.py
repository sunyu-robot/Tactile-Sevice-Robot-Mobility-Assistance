...
class Poses:
    ...
    def __init__(self, keypoints, confidence):
        ...
        self.bbox = Poses.get_bbox(self.keypoints)
        self.id = None
        self.filters = [[OneEuroFilter(), OneEuroFilter()] for _ in range(Poses.num_kpts)]

    @staticmethod
    def get_bbox(keypoints):
        found_keypoints = np.zeros((np.count_nonzero(keypoints[:, 0] != -1), 2), dtype=np.int32)
        found_kpt_id = 0
        for kpt_id in range(Poses.num_kpts):
    ...
    def update_id(self, id=None):
        self.id = id
        if self.id is None:
            self.id = Poses.last_id + 1
            Poses.last_id += 1

    def draw(self, img):
        assert self.keypoints.shape == (Poses.num_kpts, 2)
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1:
                ...
                cv2.circle(img, (int(x_a), int(y_a)), 3, Poses.color, -1)
            kpt_b_id = BODY_PARTS_KPT_IDS[part_id][1]
            global_kpt_b_id = self.keypoints[kpt_b_id, 0]
            if global_kpt_b_id != -1:
                x_b, y_b = self.keypoints[kpt_b_id]
                cv2.circle(img, (int(x_b), int(y_b)), 3, Poses.color, -1)
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                cv2.line(img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), Poses.color, 6)
...
def get_similarity(a, b, threshold=0.5):
    ...
    for kpt_id in range(Poses.num_kpts):
        if a.keypoints[kpt_id, 0] != -1 and b.keypoints[kpt_id, 0] != -1:
            ...
            similarity = np.exp(-distance / (2 * (area + np.spacing(1)) * Poses.vars[kpt_id]))
...
def track_poses(previous_poses, current_poses, threshold=5, smooth=False):
    ...
    for current_pose in current_poses:
        ...
        if smooth:
            ...
            current_pose.bbox = Poses.get_bbox(current_pose.keypoints)
