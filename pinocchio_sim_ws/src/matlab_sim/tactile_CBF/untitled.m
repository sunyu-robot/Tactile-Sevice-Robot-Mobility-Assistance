...
aa = 0;bb = 0;error1 = zeros(2,N);error2 = zeros(2,N);aa2 = 0;bb2 = 0;
%% loop begin
for i = 1:N-1
    % update the jacobian matrixstates(1)
    ...
    jacobianinv = pinv(skinstates(2).jacobian_matrix);
    Nj = eye(3) - jacobianinv* skinstates(2).jacobian_matrix;
    lb = [-1;-1;-1];ub = [1;1;1];
...
%                 end
    ...
    H_first = skinstates(2).jacobian_matrix.'*skinstates(2).jacobian_matrix+0.001*eye(3);
    f_first = -skinstates(2).dx_deflection(:,i+1).'* skinstates(2).jacobian_matrix;
    u1_star(:,i) = quadprog(H_first,f_first,A,b,[],[],lb,ub);
    
    H_second = skinstates(2).jacobian_matrix_r.'*skinstates(2).jacobian_matrix_r+0.001*eye(3);
    f_second = -skinstates(2).dx_de_r(:,i+1).'* skinstates(2).jacobian_matrix_r;
%     u2_star(:,i) = quadprog(H_second+H_first,f_first+f_second,A,b,[],[],lb,ub);
    Aeq = skinstates(2).jacobian_matrix;
    beq = skinstates(2).jacobian_matrix *u1_star(:,i);
%     options = optimoptions('quadprog', 'Algorithm', 'active-set');
%     options = optimoptions('quadprog', 'ConstraintTolerance', 1e-6);
    u2_star(:,i) = quadprog(H_second,f_second,A,b,Aeq,beq,lb,ub,u1_star(:,i));    
    dq_cmd(:,i) = jacobianinv*skinstates(2).dx_deflection(:,i+1) + Nj*pinv(skinstates(2).jacobian_matrix_r*Nj)*(skinstates(2).dx_de_r(:,i+1) - skinstates(2).jacobian_matrix_r*jacobianinv*skinstates(2).dx_deflection(:,i+1));
%     dq_cmd(:,i) = u2_star(:,i);
    error1(1,i) = norm(skinstates(2).jacobian_matrix*dq_cmd(:,i) - skinstates(2).dx_deflection(:,i+1))^2;
    error1(2,i) = norm(skinstates(2).jacobian_matrix_r*dq_cmd(:,i) - skinstates(2).dx_de_r(:,i+1))^2;
    aa = aa + error1(1,i);
    bb = bb + error1(2,i);
    error2(1,i) = norm(skinstates(2).jacobian_matrix*u2_star(:,i) - skinstates(2).dx_deflection(:,i+1))^2;
    error2(2,i) = norm(skinstates(2).jacobian_matrix_r*u2_star(:,i) - skinstates(2).dx_de_r(:,i+1))^2;
    aa2 = aa2 + error2(1,i);
    bb2 = bb2 + error2(2,i);
    q_real(:,i+1) = u2_star(:,i)*h + q_real(:,i);
    t(i+1) = t(i)+h;
end
% draw figure
figure(1)
subplot(3,1,1);
plot(t,dq_cmd(1,:),'-','linewidth',2),title('q_0');hold on;
plot(t,u2_star(1,:),'-','linewidth',2),title('q_0');hold on;
subplot(3,1,2);
plot(t,dq_cmd(2,:),'-','linewidth',2),title('q_1'   );hold on;
plot(t,u2_star(2,:),'-','linewidth',2),title('q_1'   );hold on;
subplot(3,1,3);
plot(t,dq_cmd(3,:),'-','linewidth',2),title('q_2');hold on;
plot(t,u2_star(3,:),'-','linewidth',2),title('q_2');hold on;
legend('dq_{cmd}','{u2_star}');
% figure(2)
...
figure(6)
subplot(2,1,1);
plot(t,error1(1,:),'-','linewidth',2),title('pos');hold on;
plot(t,error2(1,:),'-','linewidth',2),title('pos');hold on;
subplot(2,1,2);
plot(t,error1(2,:),'-','linewidth',2),title('rotation'   );hold on;
plot(t,error2(2,:),'-','linewidth',2),title('rotation'   );hold on;
legend('dq_{cmd}','u2_{star}');