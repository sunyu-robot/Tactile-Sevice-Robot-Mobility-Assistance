...
zeros(2,N),'x_new_desire',zeros(2,N)),1,2);
...
for i = 1:N-1
    ...
    for j = 1:2
        ...
        skinstates(j).weight(i) =  prindex(force(:,j,i),skinstates(j).dx_deflection(:,i));
        
        H_first = h*h*skinstates(j).weight(i)*skinstates(j).jacobian_matrix'*skinstates(j).jacobian_matrix + H_first;
        f_first = -h*(skinstates(j).weight(i)*skinstates(j).x_track_error(:,i).'* skinstates(j).jacobian_matrix) + f_first;
    end
    u1_star(:,i) = quadprog(H_first,f_first,A,b,[],[],lb,ub);
    %second priority
    dq_desire = [-1;0;0];
    H_second = eye(3); f_second = -dq_desire; Aeq = []; beq = [];
    for j = 1:2
