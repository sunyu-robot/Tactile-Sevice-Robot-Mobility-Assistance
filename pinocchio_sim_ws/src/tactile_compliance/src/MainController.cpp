...
void Tactile_HQP::init()
{
    if (!nh.getParam("JOINT_LIMIT", q_limit)) {
        ROS_ERROR_STREAM("can't get joints limit"); ros::shutdown();
    }
    if (!nh.getParam("VELOCITY_LIMIT", velocity_limit)) {
        ROS_ERROR_STREAM("can't get velocity limit"); ros::shutdown();
    }

    vector<double> _init_left_joints_position;
    if (!nh.getParam("INIT_LEFT_JOINTS_POSITION", _init_left_joints_position)) {
        ROS_ERROR_STREAM("can't get init left joints position!"); ros::shutdown();
    }
    vector<double> _init_right_joints_position;
    if (!nh.getParam("INIT_RIGHT_JOINTS_POSITION", _init_right_joints_position)) {
        ROS_ERROR_STREAM("can't get init right joints position!"); ros::shutdown();
    }
    init_joints_pose << 0, 0, 0,
                        Map<Vector6d>(_init_left_joints_position.data()),
                        Map<Vector6d>(_init_right_joints_position.data());

    vector<double> _Mx, _Bx, _Kx;
    if (!nh.getParam("MX", _Mx)) { ROS_ERROR_STREAM("can't get Mx"); ros::shutdown(); }
    if (!nh.getParam("BX", _Bx)) { ROS_ERROR_STREAM("can't get Bx"); ros::shutdown(); }
    if (!nh.getParam("KX", _Kx)) { ROS_ERROR_STREAM("can't get Kx"); ros::shutdown(); }
    ...
    // OSQP: 15 DOF, 815 constraints (800 CBF + 15 joint limits)
    // warm start disabled — enabling it caused core dumps in testing
    ...
    I15d.setIdentity();
    for (int i = 0; i < 815; i++) { lowerBound[i] = -1; upperBound[i] = 1; }
...
{
    // Reorder from /joint_states topic order to internal 15-DOF convention:
    // [base_x, base_y, base_z, L_shoulder_pan..L_wrist_3, R_shoulder_pan..R_wrist_3]
    q_real  << input->position[12], input->position[13], input->position[14],
               input->position[2],  input->position[1],  input->position[0],
               input->position[3],  input->position[4],  input->position[5],
               input->position[8],  input->position[7],  input->position[6],
               input->position[9],  input->position[10], input->position[11];

    dq_real << input->velocity[12], input->velocity[13], input->velocity[14],
               input->velocity[2],  input->velocity[1],  input->velocity[0],
               input->velocity[3],  input->velocity[4],  input->velocity[5],
               input->velocity[8],  input->velocity[7],  input->velocity[6],
               input->velocity[9],  input->velocity[10], input->velocity[11];
...
void Tactile_HQP::robotInitPose(const Vector15d& input)
{
    ...
    while (err.norm() > 0.01) {
        ros::spinOnce();
        dq_cmd = 0.5 * err;
        dq_cmd = dq_cmd.cwiseMax(-velocity_limit).cwiseMin(velocity_limit);
        publishCommand();
        loop_rate.sleep();  // sleep(0.2) truncates to 0 s in C — caused runaway commands and jitter
        err = input - q_real;
    }
}
...
void Tactile_HQP::getCommand()
{
    ...
    // Layer 1: tactile position tracking
    tactile_compliance.tactileTrack(q_real, dq_real, gradient_vector, Heq, dq_cmd);
    hessian        = Heq + 0.1 * I15d;  // regularisation to ensure positive definiteness
    hessian_first  = hessian.sparseView();
    gradient_first = gradient_vector.sparseView();

    collision_constraint.updateConstraintsMatrix(20, point_sphere, LinearConstraintsMatrix, lowerBound, upperBound, q_real);
    ...
    auto t_qp_start = std::chrono::steady_clock::now();
    if (solver.solveProblem() != OsqpEigen::ErrorExitFlag::NoError) ros::shutdown();
    auto t_qp_end = std::chrono::steady_clock::now();
    qp_solve_time_ms_ = std::chrono::duration<double, std::milli>(t_qp_end - t_qp_start).count();
    ...
    // Layer 2 (rotation tracking) is currently disabled
...
void Tactile_HQP::spin()
{
    ...
    while (ros::ok()) {
        ...
        bool was_tracking = tactile_compliance.tracking_active_;
        ...
        if (was_tracking) {
            qp_sum_ms_ += qp_solve_time_ms_;
            qp_count_  += 1.0;
        }

        // tracking just converged this frame
        if (was_tracking && !tactile_compliance.tracking_active_ && qp_count_ > 0) {
            std::ofstream f(perf_log_path_, std::ios::app);
            f << "| " << std::fixed << std::setprecision(3) << ros::Time::now().toSec()
              << " | event_done | - | " << qp_sum_ms_ / qp_count_ << " | "
              << tactile_compliance.last_lag_ms_ << " |\n";
            qp_sum_ms_ = 0.0;
            qp_count_  = 0.0;
        }
