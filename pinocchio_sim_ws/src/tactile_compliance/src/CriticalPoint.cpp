...
{
    // Default all spheres to a near-zero radius (inactive)
    for (int i = 0; i < 35; i++) {
        point_sphere[i].radius = 0.001;
        point_sphere[i].collision_check.clear();
    }

    // Right arm link spheres (0-13)
    ...
    // Right arm: check against left arm + base, and non-adjacent right-arm spheres
    for (int i = 2; i < 14; i++) {
        for (int j = 14; j < 35; j++)
    ...
    // Left arm: check against right arm + base, and non-adjacent left-arm spheres
    for (int i = 16; i < 28; i++) {
        for (int j = 0;  j < 14; j++) point_sphere[i].collision_check.push_back(j);
        for (int j = 28; j < 35; j++) point_sphere[i].collision_check.push_back(j);
        for (int k = 14; k < 28; k++) {
            if (k - i != 1 && k - i != -1)
                point_sphere[i].collision_check.push_back(k);
        }
...
{
    // Right arm: joints q[0..7] (base + right arm DOFs)
    for (int i = 0; i < 14; i++) {
    ...
    // Left arm: joints q[0..2, 9..13] (base + left arm DOFs)
    for (int i = 14; i < 35; i++) {
